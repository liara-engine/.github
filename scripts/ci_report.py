...
def render(title: str, sections: dict[tuple[int, str], list[Step]], run_url: str) -> str:
    out: list[str] = [f"## {title}", ""]

    all_steps = [step for steps in sections.values() for step in steps]

    if not all_steps:
        out += ["No CI checks reported for this run.","",f"<sub>[Full run]({run_url}) - updates automatically on each push.</sub>"]
        return "\n".join(out)

    failing = [s for s in all_steps if not s.ok]
    headline = (f"**{len(failing)} check(s) need attention.**"
                if failing else "**All CI checks passed.**")
    out += [f"{headline} - [full run]({run_url})", ""]

    for (_, section_name), steps in sections.items():
        out += [f"### {section_name}", ""]
...
def main() -> int:
    ...
    sections = load_steps(args.reports_dir) if args.reports_dir.is_dir() else {}
    body = render(args.title, sections, args.run_url)
